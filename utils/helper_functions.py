...
def permutation_of_individual_test(selected_diagnosis,
    ...
    # necesary input: diagnosis, attribute name, attribute nr, donor ids table x for disease, table1 occurence,
    
    # df5 = dataframe, occurence of attribute per donor, also contains main diganosis
    # mean_df = mean occurence per attribute per diagnosis
    # nr_donors_with_d = number, how many donors to randomly pick.
    # t1o = table 1 occurence
    ...
    p=perms
#     print(p)
    attribute_name = df5.columns[attribute_number]
#     print(attribute_name )
    nr_donors_with_d = int(nr_donors_with_d)
    ##Collect permuted mean-values in pR
    pR = []
    
    ## other option
    
    ## remove donors with disease x from background t10 dataframe
    donorIDS_with_disease = df5[df5['neuropathological_diagnosis'] == selected_diagnosis ]
    donorIDS_with_disease = list(donorIDS_with_disease.DonorID)
    table1_without_dd = t1o[~t1o.DonorID.isin(donorIDS_with_disease)]
    table1_without_dd = table1_without_dd[['DonorID','neuropathological_diagnosis',attribute_name]]
    
#     ## remove donors with disease x from occurence dataframe
#     occurence_without_d = df5[df5['Main_diagnosis'] != selected_diagnosis ]
#     occurence_without_d = occurence_without_d[['NBB_nr','Main_diagnosis',attribute_name]]

    ## loop over p
    for i in range(0,p):
        # pick n random rows --> df
        occurence_without_d_random = table1_without_dd.sample(n=nr_donors_with_d,replace=False)
...
#         elif m_or_m == 'median':
#             attribute_average = occurence_without_d_random[attribute_name].median()
#             pR.append(attribute_average)
